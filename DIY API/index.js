...
app.get("/random", (req, res) => {
  res.render("index.ejs", { content: JSON.stringify(jokes[Math.floor(Math.random() * jokes.length)]) });
});
//2. GET a specific joke
app.get("/specific", (req, res) => {
  specific = true;
  res.render("index.ejs", { content: "Enter id or Type", specific: specific });
});
app.post("/specific", (req, res) => {
  ...
  try {
    ...
    if (req.body.type.length) {
      const filteredData1 = jokes.filter((joke) => !joke.jokeType.localeCompare(req.body.type));
      if (req.body.id.length) {
        filteredData = filteredData1.find((joke) => joke.id == req.body.id);
      } else filteredData = [...filteredData1];
    }
    if (req.body.id.length) {
      let filteredData1 = jokes.find((joke) => joke.id == req.body.id);
      if (filteredData1) {
      ...
      if (req.body.type.length) {
        if (!req.body.type.localeCompare(filteredData.jokeType)) {
        } else filteredData = [];
      }
    }

    if (filteredData.length !== 0) res.render("index.ejs", { content: JSON.stringify(filteredData) });
    else res.render("index.ejs", { content: "No data found" });
  } catch (error) {
    console.log(error);
  }
});
//2.1 GET a specific joke by url
app.get("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const foundJoke = jokes.find((joke) => joke.id === id);
  res.render("index.ejs", { content: JSON.stringify(foundJoke) });
});

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
  console.log(req.query.type);
  const filteredData = jokes.filter((joke) => !joke.jokeType.localeCompare(req.query.type));
  if (filteredData.length !== 0) res.render("index.ejs", { content: JSON.stringify(filteredData) });
  else res.render("index.ejs", { content: "No data found" });
});
...
app.get("/postJoke", (req, res) => {
  res.render("index.ejs", { content: "Enter Joke and its Type", postJoke: true });
});
app.post("/postJokes", (req, res) => {
  console.log(req.body);
  jokes.push({ id: jokes.length + 1, jokeText: req.body.jokeText, jokeType: req.body.type });
  res.render("index.ejs", { content: "Joke entered successfully!" });
});
app.get("/jokes", (req, res) => {
  console.log(req.query);
  jokes.push({ id: jokes.length + 1, jokeText: req.query.text, jokeType: req.query.type });
  res.render("index.ejs", { content: "Joke entered successfully at" + (jokes.length + 1) + "!" });
});
...
var jokes = [
  ...
  {
    id: 9,
    jokeText: "What do you call fake spaghetti? An impasto!",
    jokeType: "Food",
  },
  {
    id: 10,
